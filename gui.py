...
class SeatEditor(ctk.CTkFrame):
    ...
    def _do_ping(self):
        ...
        def work():
            ok, msg, elapsed = council.ping_seat(cfg, sid, timeout=45)
...
class App(ctk.CTk):
    ...
    def _build_settings_tab(self):
        ...
        # 滚轮限速且仅悬停本页生效：默认 ctk 全局绑定步进过大且跨标签页乱跳
        def _settings_wheel(event):
            outer._parent_canvas.yview_scroll(int(-event.delta / 120), "units")

        def _wheel_on(_e):
            outer.bind_all("<MouseWheel>", _settings_wheel)

        def _wheel_off(_e):
            outer.unbind_all("<MouseWheel>")

        outer.bind("<Enter>", _wheel_on)
        outer.bind("<Leave>", _wheel_off)
